		...
		if(section->sh_type == SHT_REL) {
			...
			for(idx = 0; idx < section->sh_size / section->sh_entsize; idx++) {
				Elf32_Rel *reltab = &((Elf32_Rel *)((int)getEhdr() + section->sh_offset))[idx];
                                ...
                                int nameidx = (reltab->r_info & 0xffffff00)/0x100;
...
void examinElf(){
    ...
    for (int i=0;i<getEhdr()->e_shnum;i++){
        ...
        printf("%d", (getSectionByIndex(i)->sh_size));
        printf("\n");
    }
    printf("\n");
...
void printSection(){
	...
	for (int i=0;i<getEhdr()->e_shnum;i++){
    	printf("%03d  \t%-15s \t%08X \t%08X \t%08X \t %x\n",i,getNameOfSection(i),getSectionByIndex(i)->sh_addr,getSectionByIndex(i)->sh_offset,getSectionByIndex(i)->sh_size, getSectionByIndex(i)->sh_type);
    }
   	printf("\n");
...
void displaymenu(){
        ...
        printf("4-Link Check\n ");
        printf("5-Relocation Tables - Raw\n ");
        printf("6-Relocation Tables - Semantic\n ");
        printf("7-Quit\n");
}
...
int main(int argc, char *argv[]) {
    ...
    fun_desc funArray[] = {
        ...
        {"Link Check",linkTo},
        {"Relocation Tables - Raw", printRaw},
        {"Relocation Tables - Semantic", printSemantic},
        {"Quit" ,quitProg},
        {NULL ,NULL}
    };
    while(1){
        ...
        displaymenu();
        scanf("%d" , &_userChoice);
        if (_userChoice>=0&&_userChoice<=7)
