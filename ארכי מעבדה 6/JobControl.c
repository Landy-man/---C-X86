...
void updateJobList(job **job_list, int remove_done_jobs) {
    ...
    int stat;
    while (joblst) {
        int j = waitpid((-1)*joblst->pgid, &stat, WNOHANG);
        if (j == joblst->pgid || j == -1) {
            joblst->status = DONE;
        }
        
        if (joblst->status == DONE) {
            if (remove_done_jobs == TRUE) {
                printf("[%d]\t %s \t\t %s", joblst->idx, statusToStr(joblst->status), joblst->cmd);
                job *tmp = joblst->next;
                removeJob(job_list, joblst);
                joblst = tmp;
            }
        }
        joblst = joblst->next;
...
void runJobInForeground(job **job_list, job *j, int cont, struct termios *shell_tmodes, pid_t shell_pgid) {
    int ans = waitpid(j->pgid, NULL, WNOHANG);
    switch (ans) {
        case -1:
