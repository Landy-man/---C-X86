...
int readWrite(int OP_CODE,int _file, char * buffer,int _size){
    ...
    if (_inDebug==0)
        ...
        system_call(SYS_WRITE,STDERR,itoa(OP_CODE),5);
...
int main (int argc , char* argv[], char* envp[])
{
    ...
    ent * _entPointer;
    char * _path=".", *_name;
    int _fileDisc,_count,i,_bpos,_filter=0;
    ...
    if (_inDebug){
            ...
            for (i = 0 ; i < argc ; i++){
                system_call(SYS_WRITE,SRDERR,"argv[", 5);
            
                system_call(SYS_WRITE,SRDERR,itoa(i),strlen(itoa(i)));
                system_call(SYS_WRITE,SRDERR,"] = ",4);
                system_call(SYS_WRITE,SRDERR,argv[i],strlen(argv[i]));
                system_call(SYS_WRITE,SRDERR,"\n",1);
            }
    }
    ...
    for (_bpos=0; _bpos<_count;){
        ...
        if(_filter==0 || *_name==_entPointer->buf[strlen(_entPointer->buf)-1]){
